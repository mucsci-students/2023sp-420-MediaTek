...
class IdentifyBaseWord_test:
    ...
    def autoGame_test(self):
        ...
        assert randReqLetter.isalpha() & len(randReqLetter) == 1
        assert randUserLetters.isalpha() & len(randUserLetters) == 7
    
    # Tests base game functionality
    def baseGame_test(self):
        userInput = "pangrams"
        baseUserLetters, baseReqLetter = self.base.baseGame(userInput)
        assert baseReqLetter.isalpha() & len(baseReqLetter) == 1
        assert baseUserLetters.isalpha() & len(baseUserLetters) == 7
